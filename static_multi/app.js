// AquaRAG Multi-Knowledge Application Scripts
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const tabInfo = {
        "chat-tab": {
            title: "RAG 多知識對話",
            desc: "基於本地技術手冊與說明文件進行向量檢索與智慧生成"
        },
        "search-tab": {
            title: "技術手冊語意搜尋",
            desc: "利用 nomic-embed-text 語意向量進行手冊全文的高維度餘弦相似度檢索"
        },
        "explorer-tab": {
            title: "技術手冊瀏覽器",
            desc: "依據檔案名稱與段落進行分層瀏覽，檢視完整文字內容"
        },
        "stats-tab": {
            title: "資料統計與系統介紹",
            desc: "AquaRAG 多知識庫向量檢索系統架構、效能指標及收錄範圍統計"
        }
    };
    ...
    function renderTree(examSets, filterText = "") {
        ...
        if (filteredSets.length === 0) {
            treeContainer.innerHTML = `<div class="tree-loading">無匹配的內容</div>`;
            return;
        }
        ...
        filteredSets.forEach(set => {
            ...
            // Strip file extension .docx for cleaner UI
            const cleanSetName = set.exam_set.replace(/\.docx$/, "");
    ...
    async function loadQuestionDetail(qid) {
        ...
        contentPlaceholder.innerHTML = `<i class="fa-solid fa-spinner fa-spin"></i><h3>手冊段落載入中...</h3>`;
        ...
        try {
            ...
            detailExamSet.textContent = q.exam_set.replace(/\.docx$/, "");
            ...
            contentPlaceholder.innerHTML = `<i class="fa-solid fa-triangle-exclamation"></i><h3>載入失敗</h3><p>請重新點擊段落或檢查伺服器狀態。</p>`;
    ...
    async function executeSearch() {
        ...
        try {
            ...
            results.forEach(r => {
                ...
                const cleanSetName = r.exam_set.replace(/\.docx$/, "");
                
                card.innerHTML = `
                    <div class="result-meta">
                        <span class="result-badge">${cleanSetName}</span>
                    ...
                    <h3>${r.question_title}</h3>
                    <div class="result-preview">${r.full_content.substring(0, 150) + "..."}</div>
                `;
                
                card.addEventListener("click", () => {
                    openModal(r.exam_set, r.question_title, r.full_content);
                });
    ...
    async function submitChat() {
        ...
        const aiMsgDiv = appendMessage("assistant", "", true);
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error("伺服器端錯誤，請確認本機 Ollama 服務與 qwen2.5 模型是否正常啟動。");
            }
            ...
            if (state.currentCitations.length > 0) {
                ...
                chipsDiv.innerHTML = `<span class="citation-label"><i class="fa-solid fa-book-bookmark"></i> 檢索手冊來源：</span>`;
                
                state.currentCitations.forEach((cit, idx) => {
                    const chip = document.createElement("button");
                    chip.className = "citation-chip";
                    const cleanSet = cit.exam_set.replace(/\.docx$/, "").substring(0, 15) + "...";
                    chip.innerHTML = `[${idx+1}] ${cleanSet}`;
                    chip.title = `${cit.exam_set} - ${cit.question_title}`;
    ...
    function renderCitations(citations) {
        ...
        citations.forEach((cit, index) => {
            ...
            const cleanSetName = cit.exam_set.replace(/\.docx$/, "");
            
            card.innerHTML = `
                <div class="cit-meta">
                    <span>來源 [${index+1}]：${cleanSetName}</span>
    ...
    function openModal(examSet, title, content) {
        modalExamSet.textContent = examSet.replace(/\.docx$/, "");
    ...
    async function openModalFromId(qid) {
        try {
            const res = await fetch(`/api/question/${qid}`);
            const q = await res.json();
            openModal(q.exam_set, q.question_title, q.full_content);
        } catch (e) {
